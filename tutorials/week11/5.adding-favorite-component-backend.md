...
+         return res.status(500).json({ error: "Session error" });
+       }
+       console.log('Updated favorites:', req.session.user.favorites);
...
To configure Nodemon to ignore session files, you can use a `nodemon.json` file in the root of your project.

```json
{
  "ignore": ["sessions/", "**/sessions/*"]
}
```
