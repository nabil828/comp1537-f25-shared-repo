## Intro
When a user visits a website, the server doesn’t remember them by default;  HTTP is stateless!

A session is a way to make the server remember information about a user between requests.

> Definition:
A session is a temporary, server-side storage that keeps track of a user's data (like login status or shopping cart) across multiple requests.
